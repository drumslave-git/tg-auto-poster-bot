...
export function Countdown({ status }: { status: Status }) {
  ...
  const blocked = Boolean(stats.blocked) && !idle && !paused;
  const due = !paused && !idle && !blocked && remaining <= 0;
  ...
  return (
    <div className="rounded-2xl border border-slate-800 bg-gradient-to-br from-slate-900 to-slate-900/40 p-6">
      ...
      >
        {paused ? '⏸ paused' : idle || blocked ? '--:--:--' : formatDuration(remaining)}
      </div>
      <div className="mt-2 text-sm text-slate-400">
        {paused
          ? `Automatic posting is off — ${
              idle ? 'the queue is empty' : `${stats.queueCount} waiting`
            }. Resume to start the countdown.`
          : idle
